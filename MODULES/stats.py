...
This script will open a specified data file and perform a statistical analysis to determine
the volatility of the data: use this to set the parameters for the deadtime filter
"""
...
import os
PATH = os.getcwd()
data = np.recfromcsv(os.path.join(PATH,'../DATA/LJdata_1638805752.csv'),comments='#',delimiter=';',skip_header=1,names=True)
...
#        stat['%s_change'%dat] = (stat['%s_max'%dat]-stat['%s_min'%dat])/stat['%s_std'%dat]
    for dat in temp:
        grad = np.gradient(data[dat]/stat['%s_mean'%dat],data['timestamp'])
