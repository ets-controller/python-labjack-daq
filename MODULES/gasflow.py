...
def main(handle):
    ...
    aAddresses['GFS'] = []  # [Gas flow sensor FS2012]
    aAddresses['Voltage_GFS'] = [] # [GFS voltage]
    aDataTypes['GFS'] = [] # [Data types for flow and voltage measurements]
    ###########################################################################
    # AIN for GFS
    AINp = 10
    #AINn = AINp+1
    # Converts the GFS Voltage to SLPM
    VAL = [1,0.4,-0.4*0.39409]#(0.045+0.34909)]
    #VAL = [1,1,0]
    TYPE = [1,3,3]
    ...
    aAddresses['GFS'].append(7000+2*AINp)
    aAddresses['Voltage_GFS'].append(7300+2*AINp)
    aDataTypes['GFS'].append(ljm.constants.FLOAT32)
