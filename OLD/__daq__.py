...
for name in frg.names():
    ...
    for num in range(sensor['num%s'%name]):
        sensor['ident'].append(name+'%s'%(num+1))
#for name in gasflow.names():
#    sensor['names'].append(name)
#    sensor['num%s'%name] = gasflow.number(name)
#    for num in range(sensor['num%s'%name]):
#        sensor['ident'].append(name+'%s'%(num+1))
...
if __name__ == '__main__':
    ...
    #aAddresses.update(gasflow.main(LJ_dict['handle_2'])[0]);aDataTypes.update(gasflow.main(LJ_dict['handle_2'])[1]);aValues.update(gasflow.main(LJ_dict['handle_2'])[2])
    ...
    while True:
        ...
        if os.path.exists(filename):
            for name in sensor['names']:
                ...
                if name in frg.names() or name in level.names():# or name in gasflow.names():
